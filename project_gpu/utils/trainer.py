...
def fit(self):
    ...
    with open(self.checkpoint_dir + 'loss_log.txt', 'w') as loss_file:
        for epoch in range(self.hyper_params.num_epochs):
            ...
            log_message = (
                f"\nDataset: {self.hyper_params.dataset}, Epoch: {epoch}\n"
                f"Train_loss: {self.log['train_loss'][-1]:.8f}, Val_loss: {self.log['val_loss'][-1]:.8f}\n"
                f"Min_val_epoch: {np.array(self.log['val_loss']).argmin()}, "
            ...
            # Write the same log message to file
            loss_file.write(log_message)
            loss_file.flush()  # Ensure it's written to disk after each epoch
...
class ETSTGCNNTrainer(ETSequencedMiniBatchTrainer):
    ...
    def __init__(self, base_model, model, hook_func, args, hyper_params):
        ...
        predictor_model = base_model(n_stgcnn=1, n_txpcnn=5, input_feat=1, output_feat=hyper_params.num_samples,
                                     kernel_size=3, seq_len=hyper_params.k+2, pred_seq_len=hyper_params.k).to(device)
        eigentraj_model = model(baseline_model=predictor_model, hook_func=hook_func, hyper_params=hyper_params).to(device)
...
class ETSGCNTrainer(ETSequencedMiniBatchTrainer):
    ...
    def __init__(self, base_model, model, hook_func, args, hyper_params):
        ...
        predictor_model = base_model(number_asymmetric_conv_layer=7, embedding_dims=64, number_gcn_layers=1, dropout=0,
                                     ...
                                     n_tcn=5, in_dims=1, out_dims=hyper_params.num_samples)
        eigentraj_model = model(baseline_model=predictor_model, hook_func=hook_func, hyper_params=hyper_params)
        self.model = eigentraj_model
        self.set_optimizer_scheduler()
